...
export default function PublicRegistration() {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Get student data if existing
            const selectedStudent = !isNewStudent ? students.find(s => s.id === selectedStudentId) : null;
            ...
            const { data: authData, error: authError } = await supabase.auth.signUp({
                ...
                options: {
                    data: {
                        full_name: isNewStudent ? `${formData.nombre} ${formData.apellido}` : selectedStudent?.full_name,
                        nombre: isNewStudent ? formData.nombre : (selectedStudent?.nombre || ''),
                        apellido: isNewStudent ? formData.apellido : (selectedStudent?.apellido || ''),
                        rol: 'estudiante'
                    }
                }
            });
            ...
            // Wait a moment for trigger to create/update the profile
            await new Promise(resolve => setTimeout(resolve, 1000));

            // Fetch the REAL profile ID (generated by DB or existing)
            ...
            if (fetchError || !profile) {
                // Fallback: Try fetching by email if auth_id link failed
                ...
                if (emailError) throw new Error('Error al recuperar el perfil de usuario.');
                if (!profileByEmail) throw new Error('No se pudo crear el perfil de usuario.');

                // Manually link if trigger failed (Unlikely with recent fix, but safe)
                await supabase.from('usuarios').update({ auth_id: newUserId }).eq('id', profileByEmail.id);
                profile = profileByEmail;
            }

            const realStudentId = profile.id;

            const studentProfile = !isNewStudent ? students.find(s => s.id === selectedStudentId) : null;

            const profileUpdates = {
                nombre: isNewStudent ? formData.nombre : (studentProfile?.nombre || ''),
                apellido: isNewStudent ? formData.apellido : (studentProfile?.apellido || ''),
                documento_identidad: `${formData.tipoDocumento} ${formData.documento}`.trim(),
            ...
            const { data: existingEnrollment } = await supabase
                ...
                .select('id')
                .eq('curso_id', cursoId)
                .eq('estudiante_id', realStudentId)
                .single();
            ...
            // Clean up old placeholder if it was a different record (unlikely now with correct matching but good practice)
            if (!isNewStudent && placeholderUserId !== realStudentId) {
                // Only delete if it was a ghost record, but in this logic we matched by email so placeholderUserId SHOULD be realStudentId
                // We'll skip deletion to be safe unless we are sure.
            }
        ...
        <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-4 py-12">
            <div className="mb-8 text-center">
                <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    <h1 className="text-3xl font-bold text-slate-900">AcademiFlow</h1>
                </div>
                ...
                <CardContent className="pt-6">
                    ...
                    <form onSubmit={handleRegister} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="documento">Número de Documento de Identidad</Label>
                                <Input
                                    id="documento"
                                    placeholder="Ej: 1143123456"
